...
import ir.sinadalvand.searchview.graphics.SearchAnimator;
...
//                        mOnOpenCloseListener);
    ...
    void init(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mImageViewClear.setVisibility(GONE);
        ...
        mSearchEditText.setVisibility(VISIBLE); // todo
    }
...
//                });
    ...
    private void setMicOrClearIcon(boolean hasFocus) {
        if (hasFocus && !TextUtils.isEmpty(mQueryText)) {
            if (mOnMicClickListener != null) {
                mImageViewMic.setVisibility(GONE);
            }
            mImageViewClear.setVisibility(VISIBLE);
        } else {
            mImageViewClear.setVisibility(GONE);
            if (mOnMicClickListener != null) {
                mImageViewMic.setVisibility(VISIBLE);
            }
        }
    }
    ...
    private void setTextImageVisibility(boolean hasFocus) {
        if (hasFocus) {
            ...
            mImageViewImage.setVisibility(VISIBLE);
        }
    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        mShadow = ss.shadow;
        if (mShadow) {
            mViewShadow.setVisibility(VISIBLE);
        }
        if (ss.hasFocus) {
            open();
        }
